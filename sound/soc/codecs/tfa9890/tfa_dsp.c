...
//su merge the following try from old driver.
...
//su merge from old driver,for mtp0 crash.
...
int tfaRunSpeakerBoost(struct tfa98xx *tfa98xx, int force)
{
	...
	if (force || tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (tfa98xx->rev == REV_TFA9897) {
			/* TFA9897 needs dsp reset after cold start up */
			tfa98xx_dsp_reset(tfa98xx, 1);
			tfa98xx_dsp_reset(tfa98xx, 0);
		}
	} else {
...
int tfa98xx_dsp_quickstart(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
